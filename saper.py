...
class Saper:
    ...
    def przycisk(self, char):
        self._kod = char + self._kod[0:5]
        self.xyzzy()
    ...
    def ustawDane(self):
        self._szerokoscMapy, self._wysokoscMapy, self._miny = self._widok.menu.getEntryData()
        ...
        if not 2 <= self._szerokoscMapy <= 15 or not 2 <= self._wysokoscMapy <= 15 or \
            ...
            raise blad.zledane("")
        self._widok.mapa.new(self._wysokoscMapy, self._szerokoscMapy, self._miny)
        self.rysujMape()
        ...
        for x in range(self._wysokoscMapy):
            for y in range(self._szerokoscMapy):
                if self._mapaMin[x][y][0] == "M":
                    self._widok.mapa.showMinePlace(y, x, what)
    ...
    def rightClick(self, pos_x, pos_y):
        if not self._koniecGry and self._mapaMin[pos_y][pos_x][1] != "o":
            if self._mapaMin[pos_y][pos_x][1] == "f":
                ...
                self._widok.mapa.setButtonSign(pos_x, pos_y, "questionmark")
            elif self._mapaMin[pos_y][pos_x][1] == "q":
                self._mapaMin[pos_y][pos_x] = self._mapaMin[pos_y][pos_x][0] + "n"
                self._widok.mapa.setButtonSign(pos_x, pos_y, "empty")
            else:
                self._mapaMin[pos_y][pos_x] = self._mapaMin[pos_y][pos_x][0] + "f"
                self._oznaczoneMiny += 1
                if self._mapaMin[pos_y][pos_x][0] == "M":
                    self._dobrzeOznaczoneMiny += 1
                self._widok.mapa.setButtonSign(pos_x, pos_y, "flag")
            self.didWin()
            ...
            for j in range(-1, 2):
                try:
                    if y + i < 0 or x + j < 0 or self._mapaMin[y + i][x + j][1] == "f":
                        ...
                        self._widok.mapa.uncoverPlace(x + j, y + i, int(self._mapaMin[y + i][x + j][0]))
    ...
    def didWin(self):
        if self._miny == self._dobrzeOznaczoneMiny == self._oznaczoneMiny or \
                self._clearedPlaces == self._szerokoscMapy * self._wysokoscMapy - self._miny:
            self._koniecGry = True
            self._widok.mapa.win()